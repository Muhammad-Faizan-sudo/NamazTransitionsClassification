...
#firebase = firebase.FirebaseApplication("https://appinverter888-default-rtdb.firebaseio.com/", None)
new_model = tf.keras.models.load_model('TrainedModel')
new_model.summary()
...
row_content =["Transition"   "no"  "ammar"]
append_list_as_row('data.csv',row_content)       
row_content =["pitch(Y)","Roll(X)" ,"yaw(Z)"]
append_list_as_row('12.csv',row_content)
i =0
data = []
class MPU:
    ...
    def compFilter(self):
        ...
        row_contents = [rollRads]
        append_list_as_row('data_123.csv',row_contents)
    ...
    def attitudeEuler(self,index,dataToSend):
        ...
        #print('  Pitch: {:<8.1f} Roll: {:<8.1f} Yaw: {:<8.1f}'.format(self.pitch, self.roll,self.yaw))
        row_contents = [ self.pitch , self.roll,self.yaw]
        
        
        if(index<=600):
            dataToSend.append(row_contents)
            return index
        else:
            if (len(dataToSend) >= 1200):
                dataToSend = np.unwrap(dataToSend, 180)
                #dataToSend_new = movingA
                transition_detector(dataToSend[len(dataToSend)-600:len(dataToSend)])
                new = np.array(dataToSend)      
                #np.savetxt('/home/pi/program_transition/qiyam2rukhoo/9.csv', new, fmt="%f", delimiter=",")
        
                #qiyam2rukhoo,rukhoo2qoama
                print(new.shape)            
            else:
                dataToSend = np.unwrap(dataToSend, 180)
                #dataToSend_new = movingAvergefilter(dataToSend)
                transition_detector(dataToSend)
                new = np.array(dataToSend)
...
'''
def Data_pre(data):
    #extending the last number of each col of 2d array
    inp = np.array(data)
    pitch = inp[:,0]
    roll = inp[:,1]
    yaw = inp[:,2]
   
    pad_roll = np.pad(roll, (0,20), 'constant', constant_values=(roll[len(roll)-1]))
    pad_pitch = np.pad(pitch, (0,20), 'constant', constant_values=(pitch[len(pitch)-1]))
    pad_yaw = np.pad(yaw, (0,20), 'constant', constant_values=(yaw[len(yaw)-1]))
   
    DataArray = np.transpose([pad_pitch,pad_roll ,pad_yaw ])
    DataArray = DataArray[0:len(DataArray)]
    return DataArray
'''
def movingAvergefilter(graph1):   
    graph_new0 = graph1[:,0]
    graph_new1 = graph1[:,1]
    graph_new2 = graph1[:,2]
    smooth_data0 = convolve(graph_new0,kernel=Box1DKernel(5))
    smooth_data1 = convolve(graph_new1,kernel=Box1DKernel(5))
    smooth_data2 = convolve(graph_new2,kernel=Box1DKernel(30))
    DataArray = np.transpose([smooth_data0, smooth_data1, smooth_data2])
    data = prep(DataArray)
    #plt.plot(graph1)
    #plt.plot(data)
    #plt.show()
...
def prep(inp):
    ...
    roll = inp[:,1]
    yaw = inp[:,2]
    #Here i want to remove the first 20 samples and last 40 samples. To fulfill the gap of 60 sample i concatenate the last 60samples
    before = inp[30:len(inp)-40]

    after_roll = roll[470: len(roll)-60]
    after_pitch = pitch[470: len(pitch)-60]
    after_yaw = yaw[470: len(yaw)-60] 
    DataArray_a = np.transpose([after_pitch, after_roll ,after_yaw])
    DataArray = np.concatenate([before, DataArray_a])
    return DataArray

def transition_detector(data):
    brr = np.array(data)
    arr = ndimage.uniform_filter(data, size=20)
    ...
    i=0
    sample_size = 80
    for i in range(len(arr)-sample_size):
        y_roll = roll[i:i+sample_size]
        y_pitch = pitch[i:i+sample_size]
        y_yaw = yaw[i:i+sample_size]
    

        d_roll = np.diff(y_roll)
        d_pitch = np.diff(y_pitch)    
        d_yaw = np.diff(y_yaw)

        threshold = 0.9
        checkMaxPitch = np.round(max(d_pitch))
        checkMinPitch = np.round(min(d_pitch))

        checkMaxRoll = np.round(max(d_roll))
        checkMinRoll = np.round(min(d_roll))
        
        checkMaxYaw = np.round(max(d_yaw))
        checkMinYaw = np.round(min(d_yaw))
        '''
        print("checkMaxYaw", checkMaxYaw)
        print("checkMinYaw", checkMinYaw)
        print("checkMaxRoll", checkMaxRoll)
        print("checkMinRoll", checkMinRoll)
        print("checkMaxPitch", checkMaxPitch)
        print("checkMinPitch", checkMinPitch)
        '''
        
        if ( ( (checkMaxPitch >= threshold) or (checkMinPitch<= -threshold) ) and ( (checkMaxRoll >= threshold) or (checkMinPitch <= -threshold) ) ):
            print("send To Trained model")
            real_time(data,1)
            break
            
        elif ( ( (checkMaxPitch >= threshold) or (checkMinPitch<= -threshold) ) and ( (checkMaxYaw >= threshold) or (checkMinYaw <= -threshold) ) ):
            print("send To Trained model")
            real_time(data,1)
            break
            
        elif ( ( (checkMaxRoll >= threshold) or (checkMinYaw<= -threshold) ) and ( (checkMaxYaw >= threshold) or (checkMinYaw <= -threshold) ) ):
            print("send To Trained model")
            real_time(data,1)
            break
        else:
...
index = 0
state = "Qiyam"
print("State is initialize with Qiyam")
def real_time(inp,initial):
    global state
    global rakat
    global index

    states = ["Qiyam", "rukhoo", "qoama", "sajda" , "jalsa", "sajda2", "Right salam", "left salam"]
    transition_seq = [0,1,2,3,4,5,6,7]
    new = np.array(inp)
    #plt.plot(new)
    #plt.show()
    print(new.shape)
        #new_in = np.resize(new, (600, 3))
    x_test = np.round(preprocessing.normalize(new),4)
    print("real time function here")
    single_test = new_model.predict(np.expand_dims(x_test, axis=0)) 
    index_res = np.argmax(single_test)
    print(index_res)
    
    if(index_res == 0):
      print("Qiyam to rukhoo Transition")
    if(index_res == 1):
      print("Ruhoo to Qoama Transition")
    if(index_res == 2):
      print("Qoama to sajda1 Transition")
    if(index_res == 3):
      print("sajda1 to Jalsa Transition")
    if(index_res == 4):
      print("jalsa to sajda2 Transition")
    if(index_res == 5):
      print("sajda to Qiyam Transition")
    if(index_res == 6):
      print("jalsa to RS")
    if(index_res == 7):
      print("RS to LS")
        ...
        # qiyam to rukhoo state
    if (state == states[0]):
        if (index_res == transition_seq[0]):
            state = states[1] #rukhoo    
	    print(state)
        else:
            print("buzzer ON")
        
        # rukoo to qoama state
    if (state == states[1]):
        if (index_res == transition_seq[1]):
            state = states[2] #qoama    
	    print(state)
        else:
            print("buzzer ON")


        # qoama to sajda1 state
    if (state == states[2]):
        if (index_res == transition_seq[2]):
            state = states[3] #rukhoo    
            print(state)   		  
        else:
            print("buzzer ON")
        

        # sajda1 to jalsa1 state
    if (state == states[3]):
        if (index_res == transition_seq[3]):

            state = states[4] #rukhoo    
            print(state)
        else:
            print("buzzer ON")
        

        # jalsa1 to sajda2 state
    if (state == states[4]):
        if (index_res == transition_seq[4]):
            state = states[5] #rukhoo    
	    print(state)

        elif (index == 1 and index_res == transition_seq[6]):
            state = states[6] #RS
            print(state)
        ...
        # sajda2 to qiyam or jalsa2 state
    if (state == states[5]):
        if (index_res == transition_seq[5] and rakat == 0):
            state = states[0] 
            print(state)
            rakat = rakat + 1
        elif (index_res == transition_seq[3] and rakat == 1): # if classNN == 7 our next state is jalsa2
            state = states[4]
	    index = index + 1
            print(state)
        else:
        ...
        # R salam  to left salam state
    if (state == states[6]):
        if (index_res == transition_seq[7]):

            state = states[7] #rukhoo    
            print(state)
        else:
            print("buzzer ON")

            #left salam Salam state 
    elif (state == states[7]):
        rakat = rakat + 1
        print("sucessfully completed two rakat Namaz")
...
def main():
    ...
    DataToSend = []
    '''
    initialize = real_time(DataToSend,index)    #for initialize the state of namaz with Qiyam
    if (initialize == 1):
        print("state is initialize with Qiyam")
    '''   
    # Run until stopped
    try:
        while(True):
            ...
            # Print results to screen
            index_checker = mpu.attitudeEuler(index, DataToSend)
            if(index_checker>600):   
                index = 0
                print("600 samples are completed")         
            else:
                pass   
